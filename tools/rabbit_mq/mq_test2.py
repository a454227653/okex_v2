...
channel.queue_bind(exchange='routing', queue=queue_name, routing_key='test.*')
print(' [*] Waiting for logs. To exit press CTRL+C')

def callback2(ch, method, properties, body):
    print("callback2 [x] %r" % body)


channel.basic_consume(
    queue=queue_name, on_message_callback=callback2, auto_ack=False)
